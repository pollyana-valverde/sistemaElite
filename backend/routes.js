...
router.get('/funcionario/:idFuncionario', (req, res) => {
  const { id } = req.params;
  connection.query('SELECT * FROM funcionario WHERE idFuncionario = ?', [id], (err, results) => {
    if (err) {
      console.error('Erro ao buscar o registro:', err);
      res.status(500).json({ error: 'Erro ao buscar o registro' });
      return;
...
router.post('/funcionario', (req, res) => {
  const { nome, email, cpf, endereco, telefone, senha } = req.body;
  connection.query('INSERT INTO funcionario (nome, email, cpf, endereco, telefone, senha) VALUES (?, ?, ?, ?, ?, ?)', 
    [nome, email, cpf, endereco, telefone, senha], (err, result) => {
    if (err) {
      console.error('Erro ao criar o registro:', err);
      res.status(500).json({ error: 'Erro ao criar o registro' });
...
router.put('/funcionario/:idFuncionario', (req, res) => {
  const { id } = req.params;
  const { nome, email, cpf, endereco, telefone, senha } = req.body;
  connection.query('UPDATE funcionario SET nome = ?, email = ?, cpf = ?, endereco = ?, telefone = ?, senha = ? WHERE idFuncionario = ?', 
    [nome, email, cpf, endereco, telefone, senha, id], (err, result) => {
    if (err) {
      console.error('Erro ao atualizar o registro:', err);
      res.status(500).json({ error: 'Erro ao atualizar o registro' });
